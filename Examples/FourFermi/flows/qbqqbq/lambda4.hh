...
template <typename REG> class lambda4_kernel;
#include "../def.hh"
...
namespace DiFfRG
{
  namespace Flows
  {
    class lambda4_integrator
    {
    public:
      lambda4_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes, const double x_extent,
                         const JSONValue &json);
      lambda4_integrator(const lambda4_integrator &other);
      ~lambda4_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of lambda4_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda4_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG