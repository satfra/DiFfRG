...
#include "lambda4.hh"
#include "lambda4.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    lambda4_integrator::lambda4_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    {
      integrator = std::make_unique<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda4_kernel<__REGULATOR__>>>(
          quadrature_provider, grid_sizes, x_extent, json);
    }

    lambda4_integrator::lambda4_integrator(const lambda4_integrator &other)
    ...
    lambda4_integrator::~lambda4_integrator() = default;

    void lambda4_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }

    std::future<double> lambda4_integrator::request_CT(
    ...
    double lambda4_integrator::get_CT(const double k, const double p0f, const double T, const double muq,
