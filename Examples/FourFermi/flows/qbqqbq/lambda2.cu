...
#include "lambda2.hh"
#include "lambda2.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    lambda2_integrator::lambda2_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    {
      integrator = std::make_unique<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda2_kernel<__REGULATOR__>>>(
          quadrature_provider, grid_sizes, x_extent, json);
    }

    lambda2_integrator::lambda2_integrator(const lambda2_integrator &other)
    ...
    lambda2_integrator::~lambda2_integrator() = default;

    void lambda2_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }

    std::future<double> lambda2_integrator::request_CT(
    ...
    double lambda2_integrator::get_CT(const double k, const double p0f, const double T, const double muq,
