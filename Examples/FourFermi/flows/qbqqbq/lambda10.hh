...
template <typename REG> class lambda10_kernel;
#include "../def.hh"
...
namespace DiFfRG
{
  namespace Flows
  {
    class lambda10_integrator
    {
    public:
      lambda10_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
                          const double x_extent, const JSONValue &json);
      lambda10_integrator(const lambda10_integrator &other);
      ~lambda10_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of lambda10_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda10_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG