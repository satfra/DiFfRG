...
#include "lambda5.hh"
#include "lambda5.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    lambda5_integrator::lambda5_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    {
      integrator = std::make_unique<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda5_kernel<__REGULATOR__>>>(
          quadrature_provider, grid_sizes, x_extent, json);
    }

    lambda5_integrator::lambda5_integrator(const lambda5_integrator &other)
    ...
    lambda5_integrator::~lambda5_integrator() = default;

    void lambda5_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }

    std::future<double> lambda5_integrator::request_CT(
    ...
    double lambda5_integrator::get_CT(const double k, const double p0f, const double T, const double muq,
