...
#include "lambda7.hh"
#include "lambda7.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    lambda7_integrator::lambda7_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    {
      integrator = std::make_unique<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda7_kernel<__REGULATOR__>>>(
          quadrature_provider, grid_sizes, x_extent, json);
    }

    lambda7_integrator::lambda7_integrator(const lambda7_integrator &other)
    ...
    lambda7_integrator::~lambda7_integrator() = default;

    void lambda7_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }

    std::future<double> lambda7_integrator::request_CT(
    ...
    double lambda7_integrator::get_CT(const double k, const double p0f, const double T, const double muq,
