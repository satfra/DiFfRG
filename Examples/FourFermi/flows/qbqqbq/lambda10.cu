...
namespace DiFfRG
{
  namespace Flows
  {
    lambda10_integrator::lambda10_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    void lambda10_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }
