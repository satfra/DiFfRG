...
#include "lambda1.hh"
#include "lambda1.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    lambda1_integrator::lambda1_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes,
    ...
    {
      integrator = std::make_unique<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda1_kernel<__REGULATOR__>>>(
          quadrature_provider, grid_sizes, x_extent, json);
    }

    lambda1_integrator::lambda1_integrator(const lambda1_integrator &other)
    ...
    lambda1_integrator::~lambda1_integrator() = default;

    void lambda1_integrator::set_T(const double T, const double E) { integrator->set_T(T, E); }

    std::future<double> lambda1_integrator::request_CT(
    ...
    double lambda1_integrator::get_CT(const double k, const double p0f, const double T, const double muq,
