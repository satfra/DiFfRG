...
template <typename REG> class lambda8_kernel;
#include "../def.hh"
...
namespace DiFfRG
{
  namespace Flows
  {
    class lambda8_integrator
    {
    public:
      lambda8_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes, const double x_extent,
                         const JSONValue &json);
      lambda8_integrator(const lambda8_integrator &other);
      ~lambda8_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of lambda8_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda8_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG