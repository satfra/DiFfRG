...
template <typename REG> class lambda1_kernel;
#include "../def.hh"
...
namespace DiFfRG
{
  namespace Flows
  {
    class lambda1_integrator
    {
    public:
      lambda1_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes, const double x_extent,
                         const JSONValue &json);
      lambda1_integrator(const lambda1_integrator &other);
      ~lambda1_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of lambda1_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda1_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG