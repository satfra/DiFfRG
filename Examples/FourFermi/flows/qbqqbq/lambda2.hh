...
template <typename REG> class lambda2_kernel;
#include "../def.hh"
...
namespace DiFfRG
{
  namespace Flows
  {
    class lambda2_integrator
    {
    public:
      lambda2_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes, const double x_extent,
                         const JSONValue &json);
      lambda2_integrator(const lambda2_integrator &other);
      ~lambda2_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of lambda2_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorFiniteTq0TBB<4, double, lambda2_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG